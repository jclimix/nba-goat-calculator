...
while i < len(season_history):
    ...
    while a < len(season_history[i]):
        ...
        print(f"seasons_history current year: {season_history[i][a]}")
    ...
    if len(array_data) > 0:
        ...
        if era_r_ppg == 0:
             ...
             final_era_r_ppg = r_ppg[i] / (era_r_ppg / len(season_history[i]))
        
        # calc era assusts
        if era_r_apg == 0:
             final_era_r_apg = 0
        else:
             final_era_r_apg = r_apg[i] / (era_r_apg / len(season_history[i]))

        # calc era rebounds
        if era_r_rpg == 0:
             final_era_r_rpg = 0
        else:
             final_era_r_rpg = r_rpg[i] / (era_r_rpg / len(season_history[i]))

        # calc era steals
        if era_r_spg == 0:
             final_era_r_spg = 0
        else:
             final_era_r_spg = r_spg[i] / (era_r_spg / len(season_history[i]))

        # calc era blocks
        if era_r_bpg == 0:
             final_era_r_bpg = 0
        else:
             final_era_r_bpg = r_bpg[i] / (era_r_bpg / len(season_history[i]))

        # calc era field percentage
        if era_r_fg == 0:
             final_era_r_fg = 0
        else:
             final_era_r_fg = r_fg[i] / (era_r_fg / len(season_history[i]))

        # calc era field goals
        if era_r_fg == 0:
             final_era_r_fg = 0
        else:
             final_era_r_fg = r_fg[i] / (era_r_fg / len(season_history[i]))
        ...
        final_era_r_bpg = r_bpg[i] / (era_r_bpg / len(season_history[i]))
        final_era_r_fg = r_fg[i] / (era_r_fg / len(season_history[i]))
        final_era_r_3fg = r_3fg[i] / (era_r_3fg / len(season_history[i]))
        final_era_r_efg = r_efg[i] / (era_r_efg / len(season_history[i]))
        final_era_r_ts = r_ts[i] / (era_r_ts / len(season_history[i]))

        final_era_p_ppg = p_ppg[i] / (era_p_ppg / len(season_history[i]))
        final_era_p_apg = p_apg[i] / (era_p_apg / len(season_history[i]))
        final_era_p_rpg = p_rpg[i] / (era_p_rpg / len(season_history[i]))
        final_era_p_spg = p_spg[i] / (era_p_spg / len(season_history[i]))
        final_era_p_bpg = p_bpg[i] / (era_p_bpg / len(season_history[i]))
        final_era_p_fg = p_fg[i] / (era_p_fg / len(season_history[i]))
        final_era_p_3fg = p_3fg[i] / (era_p_3fg / len(season_history[i]))
        final_era_p_efg = p_efg[i] / (era_p_efg / len(season_history[i]))
        final_era_p_ts = p_ts[i] / (era_p_ts / len(season_history[i]))

    print(f"{i},{player_name[i]},{r_ppg},{p_ppg},{final_era_r_ppg},{final_era_p_ppg}")
